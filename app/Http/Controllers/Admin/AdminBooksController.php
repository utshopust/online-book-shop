...
class AdminBooksController extends AdminBaseController
{
    ...
    public function store(BooksCreateRequest $request)
    {
        ...
        if($pdf = $request->file('pdf'))
        {
            ...
            // dd($pdf_name);
            $request->pdf->move('assets/pdf/', $pdf_name);
            $input['pdf'] = $pdf_name;
        }
    ...
    public function moneystore(Request $request)
    {
        ...
        $randomId       = '#'.rand(4,9999);
        $moneycard=new Moneycard;
        $moneycard->title=$request->title;
        $moneycard->amount=$request->amount;
        $moneycard->card_id=$randomId;
        $moneycard->save();
       return redirect('/money/index')
            ->with('success_message', 'Maony card Created');
  }


      public function moneydestroy($id)
    {
        $book= Moneycard::findOrFail($id);
        $book->delete();
        return redirect()->back()->with('alert_message', 'Money Card deleted');
    }
      public function moneyedit($id)
    {
        $moneycard= Moneycard::findOrFail($id);
         return view('admin.books.edit', compact('moneycard'));
    }
